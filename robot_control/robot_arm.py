...
class G1_29_ArmController:
    def __init__(self, motion_mode = False, simulation_mode = False):
        ...
        self._motion_mode_ready = threading.Event()
    ...
    def _subscribe_motor_state(self):
        while True:
            ...
            if msg is not None:
                ...
                lowstate.mode_pr = getattr(msg, "mode_pr", 0)
    ...
    def get_mode_machine(self):
        ...
        return getattr(lowstate, "mode_machine", 0) if lowstate is not None else 0

    def get_mode_pr(self):
        '''Return current dds mode_pr.'''
        lowstate = self.lowstate_buffer.GetData()
        return getattr(lowstate, "mode_pr", 0) if lowstate is not None else 0

    def get_motion_mode_weight(self):
        '''Return current arm_sdk weight in [0, 1].'''
        return float(self.msg.motor_cmd[G1_29_JointIndex.kNotUsedJoint0].q)

    def is_motion_mode_ready(self):
        '''Return whether motion mode weight ramp-up has completed.'''
        return self._motion_mode_ready.is_set()

    def wait_for_motion_mode_ready(self, timeout = None):
        '''Block until motion mode weight ramp-up completes.'''
        return self._motion_mode_ready.wait(timeout=timeout)
